...
const char* WINDOWTITLE = "OpenGL Shader Sample -- Dharun Graphics";
...
Display()
{
	...
	if (kb == 1) {
		NowS0 = S_V.GetValue(nowSecs);
		NowT0 = T_V.GetValue(nowSecs);
	}
	else {
		NowS0 = NowS0;
		NowT0 = NowT0;
	...
	if (tb == 1) {
		NowRs = 0.125f * sin(2.0f * M_PI * Time);
		NowRt = 0.5f * sin(2.0f * M_PI * Time);
	}
	else {
		NowRs = NowRs;
		NowRt = NowRt;
...
InitGraphics()
{
	...
	for (float t = 0.0; t <= 10; t++) {
		...
		S_V.AddTimeValue(t, sv);
		T_V.AddTimeValue(t, tv);
